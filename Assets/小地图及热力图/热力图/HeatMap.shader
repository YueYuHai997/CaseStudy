Shader "Unlit/HeatMap"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _Hits[3 * 33]; // 接触点坐标，接触点强度
            int _HitCount = 0;  //接触点数量
            ...
            {
                float area_of_effect_size = _Range;
                float d = pow(max(0, 1.0 - distance(a, b) / area_of_effect_size), 2);
                return d;
            }

            float3 getHeatForPoxal(float weight)
            {
                ...
                for (int i = 1; i < 5; i++)
                {
                    if (weight < pointranges[i])
                    {
                        ...
                        float3 colo_controbution = color_range * ratio_over_lower_point;
                        float3 new_color = colors[i - 1] + colo_controbution;
                        return new_color;
                    }
                }
                return colors[0];
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float2 uv = i.uv;
                //uv = uv * 4.0 - float2(2.0, 2.0); //改变UV成   -2 到 2

                float totalWeight = 0;
                for (float i; i < _HitCount; i++)
                {
                ...
                float3 heat = getHeatForPoxal(totalWeight);
